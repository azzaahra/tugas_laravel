                ...
                <table class="min-w-full table-auto">
                    <thead class="justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">KODE CUSTOMER</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                    ...
                    @forelse($customers as $customer)
                        <tr class="border bg-white">
                            ...
                            <td class="px-16 py-2">{{ $customer->id_customer }}</td>
                            <td class="px-16 py-2">{{ $customer->nama_customer }}</td>
                            <td class="px-16 py-2">{{ $customer->email }}</td>
                            <td class="px-16 py-2">{{ $customer->no_hp }}</td>
                            <td class="px-16 py-2">{{ $customer->alamat }}</td>

                            <!--action untuk tombol hapus dan edit belum ditambahkan -->
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.customer.edit', $customer->id_customer) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>

                                <button onClick="destroy(this.id)" id="{{ $customer->id_customer }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                    @empty
