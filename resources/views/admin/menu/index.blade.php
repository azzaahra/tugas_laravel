@extends('layouts.app', ['title' => 'menu - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-600 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.menu.create') }}">TAMBAH</a>
            </button>
                ...
                <table class="min-w-full table-auto">
                    <thead class="justify-between">
                        <tr class="bg-gray-600 w-full">
                            <th class="px-16 py-2 text-left">
                                <span class="text-white">KODE MENU</span>
                            </th>
                            <th class="px-16 py-2 text-left">
                    ...
                    @forelse($menus as $menu)
                        <tr class="border bg-white">
                            <td class="px-16 py-2">{{ $menu->id_menu }}</td>
                            <td class="px-16 py-2">{{ $menu->nama_menu }}</td>
                            <td class="px-16 py-2">{{ $menu->deskripsi}}</td>
                            <td class="px-16 py-2">{{ $menu->harga }}</td>

                            <!--action untuk tombol hapus dan edit belum ditambahkan -->
                            <td class="px-10 py-2 text-center">
                                <a href="{{ route('admin.menu.edit', $menu->id_menu) }}" class="bg-indigo-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">EDIT</a>

                                <button onClick="destroy(this.id)" id="{{ $menu->id_menu }}" class="bg-red-600 px-4 py-2 rounded shadow-sm text-xs text-white focus:outline-none">HAPUS</button>
                            </td>
                        </tr>
                    @empty
